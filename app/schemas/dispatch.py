...
async def receive_dispatch(
    ...
    canal_de_venda = getattr(payload, "CanalDeVenda", None)
    nota_fiscal = getattr(payload, "NotaFiscal", None)
    infos_adicionais = getattr(payload, "InfosAdicionais", None)
    itens = payload.Itens or []

    # Tentativa de pegar o destinatário e remetente do primeiro item
    destinatario = None
    remetente = None
    if itens:
        frete = getattr(itens[0], "Frete", None)
        if frete:
            destinatario = getattr(frete, "Destinatario", None)
            remetente = getattr(frete, "Remetente", None)
